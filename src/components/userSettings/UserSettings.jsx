...
const UserSettings = () => {
  ...
  const onDeleteUser = async () => {
    const response = await deleteAccount({ uid: user.uid });
    if (!response.ok) return response;
    createErrorNotification('Пользователь успешно удален');
    setTimeout(() => {
      navigate('/');
      ...
      <div className="setting-wrapper">
        ...
        <div className="setting-content">
          ...
          <div className="input-wrap">
            ...
            <input
              type="email"
              placeholder={userEmail || 'Введите ваш email'}
              value={userEmail}
              readOnly
            />
          </div>
          <Button onClick={changeDataUser} text="Сохранить" />
          <Button onClick={onDeleteUser} text="Удалить аккаунт" />
        </div>
      </div>
      <ThemeSetting/>
    </div>
